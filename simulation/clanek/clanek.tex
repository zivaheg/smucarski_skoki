...
response to specific inputs. \cite{aoki1990state}

\subsection{Least squares method}

The least squares method (LS) is a regression technique that is used to 
...
between a known independent variable and an unknown dependent variable. \cite{Plestenjak_NUM}
...
\texttt{'WindTime|WindName|WindSpeed|Wind...'})
...
The remaining part \texttt{00:00:00.0000000} indicates the time of the jump in hours, minutes, seconds and milliseconds.
We converted this to a numerical value in seconds, which was then used as the time variable in the model.
The column \texttt{WindTime|WindName|WindSpeed|Wind...} contained multiple pieces of information separated by '\textbar'.
The wind was recorded on 12 sensors placed along the hill, each measuring one of the seven wind features in the order 
represented in the column name. To keep all the data and not multiply the time stamps, we split this 
column into $12 \times 6 = 72$ columns, each representing one of the wind features for one of the sensors (\texttt{sensor\_feature}).
...
\begin{description}
    ...
    between the chord line of the left/right ski and the horizontal plane in degrees.
    ...
    vertical, and the resulting speed of the athlete in km/h. \cite{virmavirta2019aerodynamics}
\end{description}
...
We also tried a hybrid approach combining SSM and Physics-informed Neural Networks (PINNs \cite{StatQuest_NN_YouTube}), where
the SSM would provide a baseline prediction, and the PINN would learn to correct any discrepancies, where it takes into
account physical properties of the system, such as the mass of the pilot, wind properties and gravitational force. \cite{Cai2021_PINN_FluidMechanics}
...
based on the environmental conditions and the starting state of the jumper. \cite{SSM_YouTube}
...
could improve model accuracy and provide more personalized predictions. \cite{muller2008performance}
...
animations of predicted jumps. While the current model is limited by the size of the dataset 
and the absence of certain athlete-specific variables, the results 
show that state-space models are a promising tool for analyzing ski jumping 
performance. The combination of predictive modeling and interactive visualization 
offers both researchers and general public new opportunities to explore how 
environmental conditions influence outcomes, laying the groundwork for future 
