...
helps reduce overfitting during model training.
...
without the added complexity of PINNs. ( MAYBE ADD ACTUAL NUMBERS HERE) 
...
Because the predictions of the model are made with matrix multiplications, 
they are very fast to compute, but also dependant on the stability of the matrices.
To ensure the stability of the model, we computed the eigenvalues of the A matrix. 
And while most of the eigenvalues had an absolute value less than one, some were slightly above one.
This indicates that the model is not fully stable, but should still produce reasonable results.
...
rhe actual movement function. The rest of the inputs are set to the average values 
