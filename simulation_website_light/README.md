# Planica Performance Lab — Light concept

Light, minimal sports-performance presentation of the production ski-jump state-space model. This directory is an independent design concept; the original dark `simulation_website` remains unchanged.
...
- Responsive light interface with an off-canvas control drawer on small screens.
...
The Vite development server uses `http://localhost:5174` so it can run alongside the original design on `http://localhost:5173`.

To compare both designs, open two VS Code terminals from the repository root:

```powershell
# Terminal 1 — original
cd simulation_website
npm run dev

# Terminal 2 — light concept
cd simulation_website_light
npm run dev
```
