...
# Shiny renders plots on the server and must not open a desktop Tk window.
matplotlib.use("Agg")
...
HILL_PROFILE = pd.read_csv("hill_profile.csv").to_numpy()
...
app_ui = ui.page_fluid(
    ...
    ui.tags.style("""
        ...
        .simulation-canvas-wrapper {
            ...
            border-radius: 8px;
            background: #dceaf5;
            box-shadow: 0 2px 8px rgba(20, 45, 65, 0.14);
        }
                ...
                ui.div(
                    ...
                    ui.div(
                        ui.tags.canvas(
                            ...
                            width="1200",
                            height="675",
                            aria_label="Animated ski jump in a three-dimensional Planica hill view",
                        ),
                        class_="simulation-canvas-wrapper"
                    ),
...
def server(input, output, session):
    ...
    @reactive.Effect
    async def update_synthetic_jump():
        ...
        coords = synthetic_df[["X [m]", "Y [m]", "Z [m]"]].values.tolist()
        await session.send_custom_message(
            "render_trajectory",
            {"coords": coords, "hill": HILL_PROFILE.tolist()},
        )
    ...
    async def start_animation_sliders():
        if data_sliders.get() is None:
            return

        df = data_sliders.get()
        coords = df[["X [m]", "Y [m]", "Z [m]"]].values.tolist()
        await session.send_custom_message("start_trajectory", {"coords": coords})
